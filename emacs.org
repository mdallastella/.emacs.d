...
** Completion
*** Corfu
    [[https://github.com/minad/corfu][Corfu]] enhances the default completion in region function with a
    completion overlay.
#+begin_src elisp
  (use-package corfu
    :disabled
...
#+end_src
*** Cape
    [[https://github.com/minad/cape][Cape]] provides a bunch of Completion At Point Extensions which can
    be used in combination with Corfu.
    #+begin_src elisp
      (use-package cape
        :disabled
        :init
    ...
    #+end_src
*** kind-icon
    [[https://github.com/jdtsmith/kind-icon][kind-icon]] adds configurable icon or text-based completion prefixes
    based on the ~:company-kind~ property.
    #+begin_src elisp
...
*** olivetti
    ...
    #+end_src
** Zetteldeft
   [[https://www.eliasstorms.net/zetteldeft/][Zetteldeft]]: Combine Zettelkasten with Deft functionalities.
*** Deft
    Setup [[https://jblevins.org/projects/deft/][deft]] package
    #+begin_src elisp
      (use-package deft
        :custom
