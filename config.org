...
#+begin_src emacs-lisp
  ...
  (use-package org
    ...
    :config
    (setq org-directory "~/cloud/Sputnik/braindump/"
          org-todo-keywords '((sequence "TODO(t)" "WAITING(w)" "DOING(g)" "|" "DONE(d)" "CANCELLED(c)"))
          org-tag-alist '(("progetti" . ?p) ("aree" . ?a) ("risorse" . ?r))
          org-log-repeat nil
          org-default-notes-file (concat org-directory "pages/inbox.org")
          org-agenda-files (list (my/org-location "journals/") (my/org-location "pages/"))
          org-archive-location (concat org-directory "pages/archive.org::datatree/")
          org-refile-targets `((,(directory-files (my/org-location "journals") t "\\.org$") . (:maxlevel . 3))
                               (,(directory-files (my/org-location "pages") t "\\.org$") . (:maxlevel . 3)))
...
#+begin_src emacs-lisp
  (defun my/today-file ()
    (let* ((file (concat org-directory "journals/" (format-time-string "%Y_%m_%d") ".org")))
      (find-file file)))
  ...
  (setq org-capture-templates
        '(("t" "Todo" entry (function my/today-file)
           "* TODO %? %^G\nSCHEDULED: %^t")
          ("i" "Inbox" entry (file "pages/inbox.org")
           "* [[%<%Y-%m-%d>]]: - %?")
          ("p" "Project note" entry (function (lambda () (my/org-headings "level=1+progetti")))
           "* %?\n")
          ("a" "Area note" entry (function (lambda () (my/org-headings "level=1+aree")))
           "* %?\n")
          ("r" "Resource note" entry (function (lambda () (my/org-headings "level=1+risorse")))
           "* %?\n")))
#+end_src
...
*** TODO org-agenda
Force org-agenda buffers to show on the right side.

#+begin_src emacs-lisp
  (add-to-list 'display-buffer-alist '((derived-mode . org-agenda-mode)
...
#+begin_src emacs-lisp
  (use-package consult-notes
    ...
    (setq consult-notes-use-rg t
          ...
          consult-notes-file-dir-sources `(("Pages" ?p ,(my/org-location "pages"))
                                           ("Journals" ?j ,(my/org-location "journals"))))
