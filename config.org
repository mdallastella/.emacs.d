...
** ellama
[[https://github.com/s-kostyaev/ellama][Ellama]] is a tool for interacting with large language models from Emacs.
  ...
  (use-package ellama
    ...
    (condition-case err
        (setq ellama-provider
              (make-llm-gemini :key my/gemini-api-key))
      (error (message "Failed to connect to Gemini: %s. Using default provider." err)
             (setq ellama-provider 'ellama-provider-dummy))))
#+end_src
