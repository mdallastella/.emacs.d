...
** Devil mode
Devil mode intercepts our keystrokes and translates them to Emacs key sequences
according to a configurable set of translation rules. For example, with the
default translation rules, when we type ~, x , f~, Devil translates it to ~C-x C-f~.
...
[[https://github.com/kickingvegas/casual-avy][Casual Avy]] is an opinionated Transient-based menu for Avy, a GNU Emacs package
